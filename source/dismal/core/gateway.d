...
import dismal.core.packets;
import dismal.models;
import dismal.core.opcodes;
...
{
    // hacky as fuck but make everything @optional
    string url;
    int shards;
...
class Gateway
{
    ...
    import dismal.logging.logger : infoLog, warnLog, errorLog;
    import dismal.endpoints : Endpoints;
...
private
    ...
    HTTPGatewayResponse connectBot(string token) {
        ...
        // Check if there is a message property on the Json object
        // since only invalid requests will have it
        auto json = res.readJson();
        writefln(json.toPrettyString);

        // if (json["message"].get!string != "undefined")
        //     return HTTPGatewayResponse("", 0, json["message"].get!string, json["code"].get!int);

        return deserializeJson!HTTPGatewayResponse(json);
    }
...
public:
    ...
    void authenticate(string token) {
        ...
        //     errorLog("failed to authenticate with discord!", true);
        //     return;
        // }
